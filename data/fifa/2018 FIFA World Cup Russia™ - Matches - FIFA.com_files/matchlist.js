  ...
  var matchList = {
    ...
    init: function () {
      ...
      if (_cfg.liveBloggingCompetition && _cfg.liveBlogging) {
        ...
        if (_cfg.liveblogUpdate && _cfg.pollingEnabled) {
    ...
    updateMatches: function () {
      ...
      for (var i = 0; i < _this.matchesUpdates.Results.length; i++) {
        ...
        if (isCuncurrentMatchOn) {
          if ((cuncurrentMatchID == _matchData.IdMatch) && _matchData.MatchStatus === 3) {
      ...
      _this.$_matches.each(function () {
        ...
        if (_mid && _liveMatches.indexOf(_mid.toString()) === -1) {
    ...
    updateScore: function (matchUnit, matchData) {
      ...
      if (_isLive || _period == "post_match" || _period == "abandoned" || _period == "full_time") {
        ...
        if (matchData.ResultType === 2) {
          var _scorePenaltyText = ['(', matchData.HomeTeamPenaltyScore, '-', matchData.AwayTeamPenaltyScore, ')'].join('');
          if (_this.lang === 'ar-SA') {
            _scorePenaltyText = ['(', matchData.AwayTeamPenaltyScore, '-', matchData.HomeTeamPenaltyScore, ')'].join('');
          }
          $(matchUnit).find(".fi-mu__penaltyscore-wrap").html(_scorePenaltyText);
        }
      ...
      if (_period && _period === "full_time") {
        ...
        if (_reasonWinClass) {
          ...
          if ($reasonWinBlock.length) {
            ...
            var _winnerCountryCode = (matchData.Winner === matchData.HomeTeam.IdTeam) ? matchData.HomeTeam.IdCountry : matchData.AwayTeam.IdCountry;
            ...
            _markupAbbr = _markupAbbr.replace(/{WinTeamCountryCode}/g, _winnerCountryCode);
